...
static inline void clone_data(message_t* clone,message_t*rcv){
    
    strncpy(clone->data.hdr.receiver,rcv->data.hdr.receiver,MAX_NAME_LENGTH+1);
    clone->data.hdr.len = rcv->data.hdr.len;
    clone->data.buf = malloc((clone->data.hdr.len)*sizeof(char));
    memset(clone->data.buf,0,sizeof(char)*clone->data.hdr.len);
    //con la memcpy c'erano problemi....
    for (int i =0;i<clone->data.hdr.len;i++){
        clone->data.buf[i]=rcv->data.buf[i];
    }
}

static inline message_t* clone_msg(message_t* rcv) {
    message_t* clone = malloc(sizeof(message_t));
    memset(clone,0,sizeof(message_t)); 
    clone->hdr.op = rcv->hdr.op;
    strncpy(clone->hdr.sender,rcv->hdr.sender,MAX_NAME_LENGTH+1);
    strncpy(clone->data.hdr.receiver,rcv->data.hdr.receiver,MAX_NAME_LENGTH+1);
    clone->data.hdr.len = rcv->data.hdr.len;
    clone->data.buf = malloc((clone->data.hdr.len)*sizeof(char));
    memset(clone->data.buf,0,sizeof(char)*clone->data.hdr.len);
